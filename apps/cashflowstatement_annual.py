...
annual_cash_layout = dbc.Container(children=[
...
# This is the data table for the cash flow statement and the plots for the free cash flow, financial and investing cash flows  
    ...
    dbc.Row([
        dbc.Col(
            dcc.Graph(id = "OpCashFlowPlot-annual",
            figure = {},
            style={'height': 450}),
            width = 6),

        dbc.Col(    
            dcc.Graph(id = "Fin&InvestCashFlowPlot-annual",
            figure = {},
            style={'height': 450}),
            width = 6)
    ]),
...
# This is the waterfall graph for the cash changes across the years  
    dbc.Row([
        dbc.Col(
            dcc.Graph(id = "NetCashChanges-annual",
            figure = {},
            style={'height': 700}), 
        width = 12)
    ]),
...
# This is the breakdown of the 3 different types of cash flow 
    dbc.Row([
        dbc.Col(
            dcc.Graph(id = "OpCashFlowBreakdown-annual",
            figure = {}),
            width = 4),

        dbc.Col(
            dcc.Graph(id = "FinCashFlowBreakdown-annual",
            figure = {}),
            width = 4),

        dbc.Col(
            dcc.Graph(id = "InvestCashFlowBreakdown-annual",
            figure = {}),
            width = 4)
...
@app.callback(
     [Output(component_id = 'cashflow-table-annual', component_property = 'columns'),
     Output(component_id = 'cashflow-table-annual', component_property = 'data'),
     Output(component_id='OpCashFlowPlot-annual', component_property='figure'),
     Output(component_id='Fin&InvestCashFlowPlot-annual', component_property='figure'),
     Output(component_id='NetCashChanges-annual', component_property='figure'),
     Output(component_id='OpCashFlowBreakdown-annual', component_property='figure'),
     Output(component_id='FinCashFlowBreakdown-annual', component_property='figure'),
     Output(component_id='InvestCashFlowBreakdown-annual', component_property='figure')],
...
def figure_callback(period_range, data_annual, state):
    ...
    cashflow_dff = pd.read_json(data_annual)
